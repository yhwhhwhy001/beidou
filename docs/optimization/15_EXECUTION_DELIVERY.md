...
- HEAD：`b2a6bce587ea13253c2b2bd42141146727333f43`；工作树包含 9 个本地实现/测试修改，未提交、未部署。
- 全量回归：`.venv/bin/pytest -q` → **1028 passed, 1 skipped**（1029 collected）。
- 覆盖率门：`.venv/bin/pytest tests/ -q --cov --cov-report=term --cov-fail-under=85` → **FAIL，56.50% < 85%**，并有 49 条 ResourceWarning。
...
- 最新本地监督状态（2026-08-09T07:10:07Z）：`PREFLIGHT/BLOCKED`、`trading_ready=false`、`passed=false`；P0 为未提交工作树和缺少 `BEIDOU_SIGNING_KEY`；9090 无服务。未读取密钥、未重启、未调用交易所写接口。
...
| G4 代码/覆盖率 | FAIL | 总覆盖率 56.50%，低于 85% 发布门 |
...
- 本轮未执行停机、重启、部署、撤单、平仓、余额/密钥变更或 Git 推送。
