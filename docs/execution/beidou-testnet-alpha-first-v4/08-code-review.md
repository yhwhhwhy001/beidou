...
- Version/diff: incident baseline
  `db2debcfed1a969e627b8a34b4e6bb89d8815184` through validated code SHA
...
| P0 | Testnet E2E | Historical live venue evidence exists, but later KeepAlive execution exceeded the bounded campaign authorization. | Repeated manifests and launchd/process evidence. | Preserve historical facts, withdraw bounded-campaign claim, and require a separately authorized bounded campaign only after all gates pass. | CONTAINED / ADMISSION BLOCKED |
| P1 | Repository release gates | The first remediation candidate had 98.04% coverage against the configured 100% gate. | Clean detached final candidate: `4612 passed`, `45891/45891`, `100.00%`. | Keep explicit tests that construct their own config/storage facts; do not rely on ignored workspace state. | FIXED |
...
| P0 | `deploy/beidou_testnet_verify.sh`, `deploy/com.beidou.testnet-verify.plist` | The verifier was deployed without `--once` under launchd `KeepAlive=true`, exceeding the authorized bounded campaign. | Live process/launchctl evidence and repeated manifests. | Remove daemon launchers, require `--once` in config, retain kill switch, add architecture regression. | FIXED / LOCALLY VERIFIED |
...
G6 self-review: `PASS_WITH_CONDITIONS`. The final diff has clean local G7
evidence and no unresolved code-review defect. Independent reviewer/acceptance
and required GitHub CI remain G8/G9 conditions, so release and a new campaign
are still `BLOCKED`. Current account risk is reconciled to zero, but the
KeepAlive incident invalidates the earlier bounded-campaign characterization.
