...
- **D-020** 实盘请求窗口 = `max(profile.history_bars, min_history_bars + 模型在 registry 参数下的 warmup)`（周级 tsmom 为 1,442），超过 1,500 启动即拒绝；`NO_ACTION` 的 hold 以上一周期的 `state.last_contributions` 为种子跨周期持久（D-005 的完整实现），与全历史回测的 forward-fill 同义（E-042）。
