...
**Gate**：G6 由 PARTIAL 升为 **PASS**（OPEN P0 = 0）。G7 仍 PARTIAL（测试矩阵未写）。硬门禁不再压制判定，**Final Decision 可升为 GO（受控执行）**——但升级的第三个条件 KILL-Q2/Q3（伪 OOS 标注与 FWER 分位数门）**尚未做**，所以本文件的 Final Decision 维持 Weak GO，改变的是它的理由：不再是"有未关闭的 P0"，而是"尺子本身还没修完"。
