...
| C-2 **[C 修订]** | 现行验证门对 Sharpe 1.0–1.4 的候选无区分力；tsmom 自身在诚实网格 + 5% FWER 门下仍 PASS 但余量 ≤ 0.04 | P0 | 若 N_eff 估计使阈值 ≥ 1.485 → tsmom 转 WEAK_PASS，§11 升级条件与 §0 改写；若相关噪声仿真通过率 > 5% → 重查公式 | E-13, E-14, **E-36**, **E-40** | **SUPPORTED（E1）；falsifier 已于 2026-09-06 触发 → tsmom 记 WEAK_PASS（§12.5）** |
...
- 升级为 GO 的条件 **[2026-09-06 改写，见 §12.5]**：原四条中的前两条已达成（KILL-Q15 CLOSED、KILL-Q1-tsmom CLOSED），第四条（KILL-Q18 合并列车）**已完成**——KILL-Q18 点名的四项（main 的 D-040 / D-041 / 304e549 与 be963ad 的 Funding 叶 + 42 carry 族）现已全部在 main 里；未并入 main 的分支只剩本分支与 `fix/launcher-reject-venueless-modes`（0b5807e，落后 main 151 个提交，不在 Q18 的清单内，另计）；**第三条按其本身的判据不成立**——KILL-Q2/Q3 的代码已交付，但用它量 tsmom 自己，诚实网格 OOS 1.485 对 N=141 的阈值 1.483–1.497（取决于用哪一次运行的零假设 SD），余量已小于阈值的估计误差，按 C-2 预登记的那条线记 **WEAK_PASS**。**因此 GO 不成立，判定维持 Weak GO。**新的升级条件：(i) 在当前构造下重新取得一个诚实网格 OOS，且它在**申报完 +16 之后的 N** 上仍以可读的余量（≥ 0.05）过分位数门；或 (ii) 操作者裁定 WEAK_PASS 足以支撑受控执行并写进附录 B。两条都需要操作者的一次显式决定，不由分析者代做。
...
**Gate**：G6 由 PARTIAL 升为 **PASS**（OPEN P0 = 0）。G7 仍 PARTIAL（测试矩阵未写）。硬门禁不再压制判定——但 Final Decision **仍维持 Weak GO**，理由第三次改写：不再是「有未关闭的 P0」，也不再是「尺子还没修完」，而是**尺子修完了，而它量出来的第一个数是自己**（§12.5：C-2 的 falsifier 触发，tsmom 的诚实网格数转 WEAK_PASS）。
...
| σ_null 取自 | 阈值达到 1.485 的 N | @N=125（上次） | @N=141（申报 +16 后） |
| --- | ---: | ---: | ---: |
| E-36（0.4430） | 128 | 1.482 | **1.497 → WEAK_PASS** |
...
1. **WEAK_PASS 不是 FAIL，书不停。**§9 第 2 步的复审触发条件是「FWER 门下**基础** tsmom（crowding 0）FAIL」；那一条没有触发，也**没有被测**——测它要花一次真实的账本额度。
2. **没有重跑 validate 去"确认"，是有意的。**跑一次就往共享账本再加一行，把阈值又推高一点；用一个会让结论更坏的观测去确认结论，只会让下一轮更难归因。上面的算术是精确的，σ 与 OOS 都是已落盘的实测值。
3. **这不是新的坏消息。**§0 早就写着「现行结论以极薄余量成立，任何 +20 次试验或一次不利的构造变更都可能翻转」。+16 就到了。修尺子之前，这句话没法变成一个可判定的数；现在可以了，而它给出的答案是不利的。

**结论**：Final Decision **维持 Weak GO**。四条升级条件里前两条已达成、第四条部分完成，**第三条按其本身的判据不成立**。新的升级路径见 §11——两条都要操作者的一次显式决定，不由分析者代做。
