...
| C-003 | 当前特征面板已被搜尽 | P1 | 加入新特征后同一搜索出现越过 tsmom 同口径基线的候选 | E-007, E-013 | **PARTIAL** |
...
| --- | --- | --- | --- | --- | --- |
| C-003 当前特征面板已被搜尽 | AC-P17-02 已跑（`mine-shortlist-20260906T080124Z`）：carry 42 个候选 **0 个正边际**，最好 −0.1128。**仍记 PARTIAL** —— M-P17-01 把裁决权给了 `validate`，操作者 2026-09-06 决定不写账本，所以两个分支都没被触发 | 未判 | — | Firewall 两条重开条件均未开 | 「够不到判决门」不等于「已判」：拿全样本读数当判决正是 KILL-P17-01 要拦的动作 |
...
- **AC-P17-02 已跑**（2026-09-06，`mine-shortlist-20260906T080124Z.json`）：口径全部自证，记账闭合，结果见 §9.3 与 `RESEARCH_LOG` P17 §七。**AC-P17-03 未跑**——操作者 2026-09-06 决定不写账本，因此 M-P17-01 的裁决未取，**C-003 仍记 PARTIAL**。
- **§7.2 Firewall 的两条重开条件都未触发**，触发它们的是 M-P17-01 的裁决而不是候选榜。
