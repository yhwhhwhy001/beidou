...
| 下一动作 | (a) **待操作者执行重启 #2**（账户已核：0 个 isolated、`multiAssetsMargin` true，构造 digest 不变，不会拒绝启动、不清零 M-010）；(b) 待裁：P20 阳性候选是否跑 `research book` 取 D-018 边际；(c) ~~B5 入口证据~~ 已量：DL-D1 不开工（入口不成立）、DL-D2 三核查通过；真要做 DL-Q6 时第一步改为「先写 metrics 对齐契约与测试」，不是吞吐实验 |
