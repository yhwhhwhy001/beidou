...
| **AC-L3 阻塞（操作者动作）** | `BEIDOU_ALERTS_WEBHOOK_URL_2` **为空**——第二告警通道从来没配过。RISK-P1 的人类确认点是「**重启 #1 前**操作者确认第二通道已收到演练告警」，而重启 #1 与 #2 都已发生。后果：DL-L2 的熔断「先说话再退出」整个压在一个 webhook 上；该 URL 挂掉时 `breaker_stop` 重抛原异常 → 非零退出 → launchd 每 60 秒撞同一堵墙（L1-03 形状）。代码是对的，缺的是运维前提 |
