...
| **AC-L3（操作者裁定 2026-09-07：只做一条通道）** | 操作者裁定不配第二通道，把飞书这条做好。**这条裁定把 `send()` 的返回值从 RISK-P1 的一半变成了全部**——而它当时是 `status_code < 300`，对着一个「读不懂 payload 就回 HTTP 200 + `{"code": 19001}`」的飞书机器人。更糟：发出去的是 Slack 的扁平 `{"text": ...}`，而飞书自定义机器人文档写的是 `msg_type`/`content`，**所以这条唯一的通道可能自配置起就是哑的**（日志里没有任何一条 alert 被尝试/送达/拒绝的记录，没有东西与之矛盾）。已修：按 URL 主机选 payload 形状；「送达」改为「provider 自己说成功」（它不说时仍信状态码，只会把假成功变成失败，反向不会）；新增 `beidou live alert-test` 演练命令（此前触发告警的唯一路径是真出故障）。**残余风险随裁定改变**：单通道时该 URL 挂掉 → `breaker_stop` 重抛原异常 → 非零退出 → launchd 每 60 秒重试（L1-03 形状）。这是设计内的兜底，不是缺陷 |
...
| 下一动作 | (a) ~~重启 #2~~ ✔ 已执行（2026-09-07 12:00 本地，PID 90505）：`min_liq_distance` 已落盘（4 可测 / 14 不可达 / 0 无估计），构造 digest 未变，落在窗口内未漏再平衡，旧进程 SIGTERM 优雅停机；(b) ~~P20 阳性候选跑 `research book`~~ 已跑，P21 裁决 REJECT（回撤，见 RESEARCH_LOG）；(c) ~~B5 入口证据~~ 已量：DL-D1 不开工（入口不成立）、DL-D2 三核查通过；真要做 DL-Q6 时第一步改为「先写 metrics 对齐契约与测试」，不是吞吐实验 |
