...
理由写在 §0；本文不改任何配置、不动账本，`reports/research/trials.jsonl` 跑前跑后未动。
...
（`research_cmd.py:2320`），只在签名已在账本里时才不计费。
...
242 = `ledger_trials` 88 + `prior_trials` 152 + `grid_trials` 2。账本行数另报：
`.ledger.ledger_rows` **116** 行 / `ledger_trials` **88** 个不同签名 / `duplicate_rows` 28
（两个口径按 `analysis-calibration.md` 第三行的规矩分开报）。

PASS 那一侧的余量是 **阈值的 2.7%**。同一份报告里 `.multiple_testing.dsr_p_value = 0.5788`，且
...
1. 「q95 −69.9% 距 −70% 预算还有 0.13pp」这个说法已被仓库自己撤回——同一条序列同 seed 重抽两次
   就差 1.03pp，余量比估计量自身的复现性小约 8 倍。已接受的措辞是「P(>50% 回撤) 大约是抛硬币，
   −70% 是声明的胃口，不是界」。
...
### 3.4 "实盘期就是留出"——但这个留出只有 19 小时，不是 11 天

`.holdout = null`，报告自己写「months 0；no tail reserved: every bar was available to this run」，
`.range.end` 是 2026-09-13，也就是**前一天**。KILL-006 的立场是实盘期即留出，这在逻辑上自洽。
问题在实盘期本身：
...
按 `backtest-expert` 的样本量要求（最低 30 笔、偏好 100+），这个留出**不是小，是没有**。
...
组合层 15（`.portfolio`）+ 退出层 13（`.exits`）+ 守卫 3（`.book_guards`）。脚本对 ≥8 直接给 0 分
...
一条都不构成证据。** 再叠加 §3.4 的构造 19 小时，这个留出对任何判断都不可用。
...
* **成本与执行假设：活着，而且余量很大。** §2。这一格北斗做得比这把尺子要求的更多。
* **多重检验与参数：mixed。** §3.1 的两个分母给相反答案，余量 2.7%；§3.5 有 2 个参数被检验过邻域，
...
| **O-3** `entry_threshold` 的台/峰：在 0.2 附近做 5 点扫描 | ~45 分钟 × 5 | **+5 笔试验** → `oos_selection.n_trials` 242 → 247，阈值 1.5493 → 约 1.554，而当前余量只有 0.0426 | 把 §3.5 的峰形定性。**代价方向对被测者不利，建议不做，或与 O-4 一起做** |
| **O-4** 把留出重新买回来：冻结构造 N 天不动 | 0 机时 | 0 | §3.4。**唯一能把留出变大的办法，且它只要求不做事** |

O-1 与 O-2 都是零账本，可以直接做。O-3 建议不做——它抬高的正是 §3.1 里余量只有 2.7% 的那道门。
...
2. **账本代价已先算**：n_trials 242 → 247，D-028 门 1.549343 → 1.551723，对 OOS 1.5919 的余量
   +0.042575 → **+0.040195**。门要到 **n = 351** 才首次越过 OOS（还有 108 笔余量），
   所以这 5 笔吃掉的是余量的 5.6%，不是"用光"。**本报告初版说"余量只有 0.0426 所以建议不做"，
   那句话把余量和翻转点混为一谈，是错的。**
3. 跑的是 `research validate`（不是 `research backtest`）：只有前者过退出层
   （`research_cmd.py:846` 的 `_overlaid`），而既有的 1.6725 / 1.5680 / 1.5585 是过了退出层的数。
   走 `research backtest` 会量到另一本书。

## O-1 结果 · 刹车的盲区已经修掉了，而且尺子换了方向；剩下的问题是覆盖率

**账本代价 0**（纯读盘）。脚本 `scratchpad/o1_r8_ruler_vs_mtm.py`，调的是仓库自己的
...
**账本代价 0**：脚本 `scratchpad/o2_per_bar_net_series.py` 直接调库
...
而是**我漏了退出层**——`validate` 在 `run_backtest` 之前先过 `_overlaid`（`research_cmd.py:846`），
...
**账本代价：+5 笔，如定价。** `trials.jsonl` 2313 → **2318** 行。
...
| | 2 点网格（现行证据） | **5 点网格（本次）** |
| --- | ---: | ---: |
| walk-forward OOS（折选混合） | 1.5919 | **1.4006** |
| **在跑那个配置自己的 OOS** | 1.5919 | **1.5916** |
| PBO | 0.2082 | **0.3662**（> 0.3 硬门） |
| 每折选中的 `entry_threshold` | — | **[0.16, 0.24, 0.24, 0.24, 0.24]** |
| verdict | PASS | **FAIL** |

两条读法，缺一不可：

1. **在跑的那个配置毫发无伤**：它自己的样本外是 1.5916，与 1.5919 只差 0.0003。
2. **但只要让样本外选择在一个参数的五个值之间挑，整件事就不过门。** 而且 5 折里有 4 折选的是
...
不是我加的）。D-028 门 1.549343 → **1.557119**，在跑那个配置的余量 +0.042575 → **+0.034798**，仍 PASS。
...
**还有 92 笔的余量。** 初版说「余量只有 0.0426，所以 O-3 建议不做」——那句话把**余量**和**翻转点**
混为一谈了：5 笔吃掉的是余量的 5.6%，不是把它用光。**初版那条建议是错的，这次的执行是对的。**
...
**所以"冻结是为了攒样本外证据"这个说法，按数量级就是错的**，初版 O-4 那句"唯一能把留出变大的办法"
...
然后冻结到 2026-10-13T19:00Z。这两件加起来 0 机时、0 账本，买回三个正在瞎着的监控。
...
**在役读数零变化**（唯一消费者带 profile），行数不变（棘轮除 `beidou_governance` 外全是 0 余量）。
...
`ruff check` 与 `ruff format --check` 干净；棘轮七个包全部 ok，源码净改动 **1 行增 1 行删**；
账本仍是 2318 行（本节未跑任何研究命令）；注册表的 `evidence` 指针一个字节未动。
