...
| 32 条候选的实际进度 | **块 1（13 条）**：metrics→Panel 的 #16/#17 ✔、现货 ingest ✔（362/528 有现货腿，解锁 #11 基差 / #14 资金费套利 / #39 期现）、公共清算流 #19 **判定不可用**（币安 USDⓈ-M 无清算历史，唯一存在的币本位归档在实盘期前 23 个月停更 → 平价**不可能满足** → RISK-G3 该列不进实盘）；外部 API 五条中**两条已处置（2026-09-09）**——**#28 解锁判「不可得」**（唯一免费源是一份持续覆写的单版本文档，无 as-of；Wayback 仅存的两份带日期快照上，`aave` **已过去**的 14,140 个点有 409 个变了、最大 5.75 倍 → 时点表无法重建 → RISK-G3 该列不进实盘，**未写下载器**）、**#31 链上 ✔**（Coin Metrics Community 免费无 key，覆盖 **49/528 = 9.3%**；流量两列因逐格 `-status-time` 最长晚写 **769 天**被拒，计数三列收；契约由 blockchain.info 独立见证证伪，**355/355 vs 97/354、97/355**）；**#29 指数 / #30 社交 / #32 宏观三条未做**。<br>**块 2（6 条）**：hod #8 ✔、OI/LS 叶 ✔、regime 叶 ✔；**basis 叶现在被现货 ingest 解锁**；liquidation 叶**永久阻塞**（无数据可喂）。<br>**块 3（6 条）：6/6 已处置**——缠论 REFUTED、配对 #6 REFUTED、regime #47 结项不实现、上新 #27 阻塞（两道门，且样本 24 次）、岭回归 #22 阻塞（需 ≥3 因子）、meta-label #24 Won't。<br>**块 4（7 条）：7/7 已处置**——冲击模型 #43 ✔（`vol_target` 重推属 Phase 4a）、动态杠杆 #49 已有、回撤节流 #20 判否、**GARCH #35 / HRP #48 / VWAP #42 三条 REFUTED**、抵押品分母已裁定。<br>**块 6（7 条）：✔ 收口（2026-09-09）**——xsmom / carry / residual 三条重测完毕**全部 FAIL**；meanrev（gross Sharpe −1.73，成本出现之前就是错的）与 breakout（成本是毛利的 1606%、换手 17× tsmom）按「改变的条件动不了死因」**不跑**；节流 09-07 已重测（0.30 下两个 universe 都不过，代价是 D-017 限额的 4–5 倍）；regime 已结项。**七条无一进 probe，全部维持 REFUTED**，共花 **12 行**账本（按字面跑七条要 56 行换 0 个新结论） |
