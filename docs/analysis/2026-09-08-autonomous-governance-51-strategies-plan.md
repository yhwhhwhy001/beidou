...
| 项 | 值 |
| --- | --- |
| Phase | Phase 0 ✔ · Phase 2 ✔ · **Phase 1 未完**（DL 表六项全交付；块 5 的五条里 `配对选择计费` **已做**（`8ca081f`，19,578 个候选配对进 `pairs_search` 桶），`事件时间对齐契约` **仍没做**——而块 5 自己写着「没它们，后面做了也不算数」，块 6 的 7 条重测因此**仍被卡住**）· **Phase 3 约走了三分之一** · **Phase 4a / 4b / 5 完全未开始**。重启 #6/#7/#8/#9 ✔ |
| 32 条候选的实际进度 | 块 1 数据管线 13 条：**2/13**（metrics→Panel 的 #16 OI、#17 多空比）——现货 ingest、公共清算流、五条外部 API 全未做。块 2 挖掘节点 6 条：**3**（hod #8、OI/LS 叶、regime 叶），basis 叶与 liquidation 叶**卡在块 1**。块 3 手写信号 6 条：**结项 2**（缠论 REFUTED、配对 #6 FAIL），#24 Won't，**#27 上新 / #47 手写 regime / #22 岭回归未建**。块 4 构造/执行 7 条：#43 冲击模型的**模型**已建（`vol_target` 重推明写未做）、#49 已有、#20 已判否，**GARCH #35 / HRP #48 / VWAP #42 / 抵押品分母未做**。块 6 的 7 条：**0**，卡在块 5 |
