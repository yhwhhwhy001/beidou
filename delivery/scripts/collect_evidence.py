...
def main() -> int:
    ...
    p.add_argument("--agent-id", default=os.getenv("BEIDOU_AGENT_ID", "unknown-agent"))
    ...
    root = Path("artifacts/evidence") / a.task
    root.mkdir(parents=True, exist_ok=True)
    stamp = dt.datetime.now(dt.timezone.utc).strftime("%Y%m%dT%H%M%S%fZ")
    out = root / f"{stamp}.stdout.log"
    err = root / f"{stamp}.stderr.log"
    manifest = root / f"{stamp}.manifest.json"
