    ...
    for t in data.get("tasks", []):
        ...
        for name in ("TASK.md", "AGENT_PROMPT.md", "ACCEPTANCE.yaml", "ROLLBACK.md"):
            if not (d / name).exists():
                fail(f"{tid}/{name} missing")
        ac = yaml.safe_load((d / "ACCEPTANCE.yaml").read_text(encoding="utf-8"))
        if ac.get("task_id") != tid:
            fail(f"{tid} acceptance id mismatch")
        if not ac.get("criteria"):
            fail(f"{tid} has no acceptance criteria")
    if len(ids) != 20 or len(set(ids)) != 20:
        fail(f"expected 20 unique tasks, got {len(ids)}")
    known = set(ids)
    for t in data["tasks"]:
        unknown = set(t.get("dependencies", [])) - known
        if unknown:
            fail(f"{t['id']} unknown deps {unknown}")
    # cycle detection
    graph = {t["id"]: t.get("dependencies", []) for t in data["tasks"]}
    visiting = set()
    visited = set()

    def dfs(n):
        if n in visiting:
            fail(f"dependency cycle at {n}")
        if n in visited:
            return
        visiting.add(n)
        for x in graph[n]:
