...
def check_factors(states, *, active_strategy_refs=None):
    results = []
    now = time.time()
    active = active_strategy_refs or set()
    for fs in states:
        if math.isnan(fs.value) or math.isinf(fs.value):
            results.append(
                MonitoringCheckResult(
                    ...
                    message=f"Factor {fs.factor_id}: value={fs.value}",
                    observed_at=now,
                )
            )
        elif fs.lifecycle in ("RETIRED", "SUSPENDED") and fs.factor_id in active:
            results.append(
                MonitoringCheckResult(
                    check_id="runtime.health.factor_lifecycle",
                    entity_type="factor",
                    ...
                    status=CheckStatus.FAIL,
                    severity=CheckSeverity.P0,
                    message=f"Factor {fs.factor_id}: {fs.lifecycle} but referenced",
                    observed_at=now,
                )
            )
            ...
            results.append(
                MonitoringCheckResult(
                    ...
                    message=f"Factor {fs.factor_id}: stale ({now - fs.last_evaluation:.0f}s)",
                    observed_at=now,
                )
            )
    return results
