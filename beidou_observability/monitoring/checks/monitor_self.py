...
def check_component_health(components=None):
    comps = components or COMPONENT_FAILURE_MATRIX
    now = time.time()
    results = []
        ...
        if not c.healthy:
            results.append(
                MonitoringCheckResult(
                    check_id="runtime.health.monitor_self",
                    entity_type="monitor_component",
                    entity_id=name,
                    status=CheckStatus.FAIL if c.safe_action == "NO_NEW_RISK" else CheckStatus.WARN,
                    severity=CheckSeverity.P0 if c.safe_action == "NO_NEW_RISK" else CheckSeverity.P1,
                    message=f"Component {name} UNHEALTHY: {c.safe_action}",
                    observed_at=now,
                )
            )
    return results
...
def check_monitor_loop_health(last_loop_at, max_stall=30.0):
    ...
    if elapsed > max_stall:
        return MonitoringCheckResult(
            ...
            message=f"Main loop STALL: {elapsed:.1f}s>{max_stall}s",
            observed_at=now,
        )
    return MonitoringCheckResult(
        check_id="runtime.health.monitor_self",
        ...
        message=f"Loop OK ({elapsed:.1f}s)",
        observed_at=now,
    )
