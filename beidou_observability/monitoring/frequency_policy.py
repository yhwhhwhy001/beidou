...
def restart_frequency_state():
    return init_frequency_state("restart_or_recovery")


def is_promotion_clean(results, allowlisted_p2_warns=None):
    allowed = allowlisted_p2_warns or set()
    for s, sev in results:
        if sev in (CheckSeverity.P0, CheckSeverity.P1) and s != CheckStatus.PASS:
    ...
    *,
    ...
    if not clean:
        ...
        return FrequencyState(
            ...
            last_check_at=now,
            next_due_at=now + 600,
            last_level_change_at=now if changed else cur.last_level_change_at,
            last_reason=cr,
            policy_version=cur.policy_version,
        )
    ...
    if cur.level == FrequencyLevel.ALERT and ns >= PROMOTION_STREAK_REQUIRED:
        return FrequencyState(
            ...
            promotion_clean_streak=0,
            last_check_at=now,
            next_due_at=now + 1800,
            last_level_change_at=now,
            last_reason=f"promoted:{ns}",
            policy_version=cur.policy_version,
        )
    if cur.level == FrequencyLevel.NORMAL and ns >= PROMOTION_STREAK_REQUIRED:
        return FrequencyState(
            level=FrequencyLevel.STABLE,
    ...
    return FrequencyState(
        ...
        promotion_clean_streak=ns,
        stable_since=cur.stable_since,
        last_check_at=now,
        next_due_at=now + cur.interval_seconds,
        last_level_change_at=cur.last_level_change_at,
        last_reason=f"streak:{ns}",
        policy_version=cur.policy_version,
    )


def is_due(state, tolerance=5.0):
    return time.monotonic() >= (state.next_due_at - tolerance)
