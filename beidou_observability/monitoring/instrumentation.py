...
class InstrumentationPolicy:
    fail_open_on_exit_paths: bool = True
    fail_closed_on_entry_paths: bool = True
    append_only: bool = True
    overhead_budget_us: int = 500


@dataclass(slots=True)
class InstrumentationRecorder:
    events: list = field(default_factory=list)
    policy: InstrumentationPolicy = field(default_factory=InstrumentationPolicy)
    ...
    def stage_completed(self, cid, stage):
        return any(e.correlation_id == cid and e.stage == stage for e in self.events)
