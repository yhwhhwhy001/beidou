...
class DecisionSnapshot:
    ...
    def verify_inputs(self, account_facts: dict, policy: dict, features: dict) -> bool:
        """独立重算验证：输入是否匹配。"""
        return (
            account_facts.get("version") == self.account_fact_version and policy.get("version") == self.policy_version
        )
...
class DecisionStore:
    ...
    def verify_all(self, account_facts: dict, policy: dict, features: dict) -> list[str]:
    ...
    def integrity_check(self) -> bool:
        """全部决策 checksum 完整性验证。"""
        return all(d.checksum == d._compute_checksum() for d in self._decisions)
