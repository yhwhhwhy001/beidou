...
| caliber | ENFORCED at N=259 (the per-strategy bucket): threshold 1.56, margin +0.03.  REPORTED and never enforced at N=1771 (the whole library): threshold 1.77, margin -0.18, p_family 0.2398.  Which N is the gate is R0 / KILL-AR-01, an operator ruling rather than a property of the arithmetic, and both numbers are printed so the ruling stays arguable from the artefact alone. |
...
| gate | max_sharpe_quantile |
| variance | 0.0000 |
| threshold_annual | 1.7674 |
| expected_max_annual | 1.5006 |
| p_family | 0.2398 |
| oos_sharpe_annual | 1.5854 |
...
| key | value |
| --- | --- |
| slip2 | oos=1.59 threshold=1.56 margin=0.03 clears=1.00 |
| slip4.43 | oos=1.54 threshold=1.56 margin=-0.01 clears=0.00 |
| slip5.5 | oos=1.53 threshold=1.56 margin=-0.03 clears=0.00 |
| slip9.2 | oos=1.46 threshold=1.56 margin=-0.10 clears=0.00 |
