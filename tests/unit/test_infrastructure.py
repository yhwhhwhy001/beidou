...
class TestBackupVerification:
    def test_valid_backup(self):
        bv = BackupVerification(
            backup_id="bkp-001", data_domain="ledger",
            restore_successful=True, ledger_balanced=True,
            intent_integrity=True, reconciliation_passed=True,
    ...
    def test_invalid_backup_missing_ledger(self):
        bv = BackupVerification(
            backup_id="bkp-002", data_domain="ledger",
            restore_successful=True, ledger_balanced=False,
...
class TestDisasterRecovery:
    ...
    def test_can_trade_after_account_restore(self):
        dr = DisasterRecoveryPlan(plan_id="dr-001", topology=InfrastructureTopology(environment="prod"))
        dr.backup_verifications.append(BackupVerification(
            backup_id="bkp-001", data_domain="account_facts",
            restore_successful=True, ledger_balanced=True,
            intent_integrity=True, reconciliation_passed=True,
...
class TestHealthProbes:
    ...
    def test_readiness_trading_eligibility_separate(self):
        probe = ReadinessProbe(can_connect_db=True, can_connect_kafka=True, can_connect_exchange=True, account_facts_verified=False, trading_eligible=False)
        assert probe.is_ready()
        assert not probe.is_trading_eligible()

    def test_trading_eligible_when_all_ok(self):
        probe = ReadinessProbe(can_connect_db=True, can_connect_kafka=True, can_connect_exchange=True, account_facts_verified=True, trading_eligible=True)
