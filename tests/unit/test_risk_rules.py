...
class TestR5R6R7:
    ...
    def test_r7_pass(self):
        assert (
            _r7_liquidation_distance(_ctx(liquidation_price=45000.0, current_price=50000.0, is_long=True))
            == RuleDecision.PASS
        )

    def test_r7_reject(self):
        assert (
            _r7_liquidation_distance(_ctx(liquidation_price=49500.0, current_price=50000.0, is_long=True))
            == RuleDecision.REJECT
        )
...
class TestR8R9R10:
    def test_r8_pass(self):
        assert _r8_protection_coverage(_ctx(protected_positions=2, total_positions=2)) == RuleDecision.PASS
    ...
    def test_r9_reject(self):
        assert _r9_account_capability(_ctx(can_trade=False, can_deposit=True, can_withdraw=True)) == RuleDecision.REJECT
...
class TestRiskRuleRegistry:
    ...
    def test_evaluate_all_pass(self):
        ...
        for rid, d in decisions.items():
            assert d in (RuleDecision.PASS, RuleDecision.UNKNOWN), f"Rule {rid} unexpectedly {d}"
