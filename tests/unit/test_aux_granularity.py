        ...
        rows.append(
            {
                ...
                "close": price, "open": price, "high": price * 1.001,
                "low": price * 0.999, "volume": 1000.0 + i, "is_closed": True,
                "mark": price, "mid": price, "vwap": price,
            }
        )
    return rows
...
def test_aux_ic_computed_on_aux_series_and_same_sign() -> None:
    ...
    aux = _momentum_klines(
        300, seed=22, drift=-0.002, vol=0.01,
        start=datetime(2024, 2, 15, tzinfo=timezone.utc), bar_hours=24,
    )
    result = runner.run(
        price_data=primary, venue="BINANCE", symbol="BTCUSDT", timeframe="1h",
        aux_price_data=aux,
    ...
    tf = [
        s
        for b in result.evidence_bundles
        for s in b.stability_results
        if s.get("dimension") == "timeframe_robustness"
    ]
    assert any(
        s.get("is_stable") is True and s.get("ic_primary", 0.0) * s.get("ic_aux", 0.0) > 0
        for s in tf
