...
check('1.1 24h Ticker', 'lastPrice' in ticker,
      f'BTCUSDT={last_price}  change={change_pct:+.2f}%  24h:[{low_24h}..{high_24h}]')

depth = api('/fapi/v1/depth', params={'symbol': 'BTCUSDT', 'limit': 10})
best_bid = float(depth['bids'][0][0]); best_bid_qty = float(depth['bids'][0][1])
best_ask = float(depth['asks'][0][0]); best_ask_qty = float(depth['asks'][0][1])
spread = best_ask - best_bid
spread_bps = spread / best_ask * 10000
check('1.2 OrderBook', len(depth['bids']) > 0,
...
if isinstance(klines, list) and len(klines) >= 10:
    ...
    check('1.3 Klines趋势', True,
          f'trend={price_trend}  vol={avg_volume:.0f}/candle  volatility={recent_volatility*100:.2f}%')
else:
    recent_volatility = 0.01; avg_volume = 1000; price_trend = 'UNKNOWN'
...
from beidou_shared.types import (VenueId, InstrumentId, VenueInstrument, Price, Quantity,
    OrderSide, OrderType, OrderId, TimeInForce, AccountId, AccountRef, CorrelationId,
    StrategyId, MonetaryValue, DataQualityTier, SchemaVersion, RiskApprovalId, RiskDecision, ResultStatus)
...
gate.checks.append(DQCheckResult(check_type=DQCheckType.COMPLETENESS, tier=DataQualityTier.PASS,
    detail=f'spread={spread_bps:.2f}bps'))
...
fs = FeatureStore()
fs.store(FeatureVector(name='btcusdt_market', values={'price': last_price, 'spread_bps': spread_bps,
    'volatility': recent_volatility, 'trend': 1 if price_trend == 'UP' else -1},
...
state = mse.estimate(VenueId('BINANCE'), InstrumentId('BTCUSDT'), {
...
cm = CostModel(); cm.set_fee_tier(VenueId('BINANCE'), 'vip1', maker_bps=2.0, taker_bps=4.0)
est = cm.estimate_order(vi, Quantity(amount='0.005'), Price(amount=str(best_bid)), OrderSide.BUY,
                         urgency=0.5, spread_bps=spread_bps)
...
if price_trend == 'UP' and recent_volatility < 0.02:
    signal_dir = 'LONG'; signal_strength = 0.7
elif price_trend == 'DOWN' and recent_volatility > 0.005:
    signal_dir = 'SHORT'; signal_strength = 0.6
elif spread_bps < 1.0:
    signal_dir = 'LONG'; signal_strength = 0.4  # 低spread做maker有优势
else:
    signal_dir = 'LONG'; signal_strength = 0.3  # default mild long bias
...
signal = AlphaSignal(
    strategy_id=StrategyId('trend_v1'), component_type=AlphaComponentType.ENTRY,
    direction=direction, strength=signal_strength, confidence=0.6 + signal_strength * 0.3,
...
print(f'\n  >>> 策略决策: {order_side} {order_qty} BTCUSDT @ {order_price} (GTC - 挂单可见)')
print(f'  >>> 理由: trend={price_trend} vol={recent_volatility*100:.1f}% spread={spread_bps:.1f}bps')
print(f'  >>> bestBid={best_bid} bestAsk={best_ask} 出价={order_price} (优于最优价)')
...
check('3.1 PreRisk检查', order_notional <= 500000,
      f'名义价值={order_notional:.0f} ≤ {500000} (max_position_notional)')
...
sm = RiskApprovalStateMachine(); sm.approve(approval_id)
...
# 幂等性
try:
    ob.commit(intent); check('4.2 幂等性', False, 'should reject')
except ValueError:
    check('4.2 幂等性', True, '重复Intent正确拒绝')

# Step 2: 向交易所下单
print(f'\n  ╔══════════════════════════════════════════════════════════╗')
print(f'  ║  👉 向 Binance Demo 下单                               ║')
print(f'  ║  Symbol: BTCUSDT                                       ║')
print(f'  ║  Side:   {order_side:<13s}  Qty: {order_qty:<10s}                ║')
print(f'  ║  Price:  {order_price:<13s}  Type: LIMIT                  ║')
print(f'  ║  ClientOrderId: {client_id}   ║')
print(f'  ╚══════════════════════════════════════════════════════════╝')

order = api('/fapi/v1/order', method='POST', signed=True, params={
    'symbol': 'BTCUSDT', 'side': order_side, 'type': order_type,
    'quantity': order_qty, 'price': order_price, 'timeInForce': order_tif,
...
print(f'  开始监控订单 {order_id}... (最长等待 {max_wait}s)')
for i in range(max_wait * 2):  # 每0.5s查一次
    time.sleep(0.5)
    ...
    if status != final_status:
        print(f'  [{elapsed:.1f}s] status: {final_status} → {status}  executed={executed}')
        final_status = status
...
if final_status == 'FILLED':
    ...
    check('5.1 订单已成交', True,
          f'status=FILLED  filled={fills[-1] if fills else 0} ({fill_pct:.0f}%)  '
          f'time={time.time()-monitor_start:.1f}s')
...
check('5.2 状态机同步', tracker.status.value in ('NEW', 'PARTIALLY_FILLED', 'FILLED'),
...
if final_status == 'FILLED':
    ...
    print(f'  订单已成交 {filled_qty} BTC，下反向单平仓...')
    print(f'  {close_side} {filled_qty} BTCUSDT @ {close_price}')

    close_client = f'beidou-close-{int(time.time()*1000)}'
    close_order = api('/fapi/v1/order', method='POST', signed=True, params={
        'symbol': 'BTCUSDT', 'side': close_side, 'type': 'LIMIT',
        'quantity': str(filled_qty), 'price': close_price, 'timeInForce': 'GTC',
    ...
    if 'error' in close_order:
        ...
        print('  平仓限价单失败，尝试市价平仓...')
        close_order = api('/fapi/v1/order', method='POST', signed=True, params={
            'symbol': 'BTCUSDT', 'side': close_side, 'type': 'MARKET',
    ...
    if 'error' not in close_order:
        ...
        check('6.1 平仓单', True,
              f'orderId={close_id}  status={close_status.get("status")}  '
              f'executed={close_status.get("executedQty")}')
    else:
        check('6.1 平仓单', False, str(close_order.get('msg', ''))[:80])
else:
    # 未成交 → 撤单
    cancel = api('/fapi/v1/order', method='DELETE', signed=True, params={
        'symbol': 'BTCUSDT', 'orderId': order_id,
    })
    tracker.apply(OrderEvent.CANCEL_REQUESTED)
    tracker.apply(OrderEvent.CANCELED)
    check('6.1 撤单', cancel.get('status') == 'CANCELED',
          f'orderId={order_id}  status={cancel.get("status")}')
...
if isinstance(open_ords, list):
    ...
    if beidou_ords:
        print(f'  清理 {len(beidou_ords)} 个遗留测试订单...')
        for o in beidou_ords:
            api('/fapi/v1/order', method='DELETE', signed=True,
                params={'symbol': o['symbol'], 'orderId': o['orderId']})
...
# 订单详情(供用户在交易所查验)
print(f'')
print(f'  交易所环境 : {REST_URL}')
print(f'  策略信号   : {signal_dir} strength={signal_strength:.2f} (trend={price_trend})')
print(f'  订单详情   :')
print(f'    Symbol       : BTCUSDT')
print(f'    Side         : {order_side}')
print(f'    Quantity     : {order_qty} BTC')
print(f'    Price        : {order_price} USDT')
print(f'    Type         : LIMIT / GTC')
print(f'    Order ID     : {order_id}')
print(f'    Client ID    : {client_id}')
print(f'    Status       : {final_status}')
print(f'    Filled       : {fill_pct:.1f}%')
print(f'')
print(f'  可在 Binance Demo 订单历史中查找: orderId={order_id}')
print(f'')
