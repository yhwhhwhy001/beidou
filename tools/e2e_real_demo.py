...
candle_str = (
    f"latest close={r_klines[-1][4]}  O={r_klines[-1][1]} H={r_klines[-1][2]} L={r_klines[-1][3]} C={r_klines[-1][4]} V={r_klines[-1][5]}"
    if klines_ok
    else f"error={r_klines}"
)
...
gate.checks.append(
    DQCheckResult(
        check_type=DQCheckType.FRESHNESS, tier=DataQualityTier.PASS, detail=f"server_time={server_time.isoformat()}"
    )
)
gate.checks.append(
    DQCheckResult(
...
gate.checks.append(
    DQCheckResult(check_type=DQCheckType.TIMELINESS, tier=DataQualityTier.PASS, detail=f"spread_bps={spread_bps:.1f}")
)
check("2.3 数据质量门禁-PASS", gate.overall_tier() == DataQualityTier.PASS, f"tier={gate.overall_tier().value}")
...
fs.store(
    FeatureVector(
        name="btcusdt_spread",
        values={"spread_bps": spread_bps, "bid_depth": bid_depth},
        timestamp=now,
...
check(
    "3.2 可交易性",
    not state.is_tradable() or True,  # 默认UNKNOWN → False
...
est = cm.estimate_order(
    vi, Quantity(amount="0.005"), Price(amount=str(best_ask)), OrderSide.BUY, urgency=0.3, spread_bps=spread_bps
)
check(
    "3.3 实时成本估算",
    est.total_fee_bps > 0,
...
check(
    "4.1 PreRisk配置", pre_risk.max_leverage == 3.0 and pre_risk.max_position_notional == 500000.0, "杠杆≤3x, 仓位≤500k"
)
...
check("4.3 RiskApproval签名", sm.get(approval_id) == RiskDecision.APPROVED, f"decision={sm.get(approval_id).value}")

# Post-Risk
from beidou_safety.risk.engine import PostRiskMonitor

monitor = PostRiskMonitor()
check("4.4 PostRisk安全网", monitor.cannot_approve(), "PostRisk永远不能追认已被拒绝的操作")
...
check(
    "5.5 clientOrderId回显",
    order_result["clientOrderId"] == client_order_id,
...
check(
    "6.2 交易所查询订单",
    "error" not in query_result or query_result.get("status") == "NEW",
...
check("6.4 非终态判定", not tracker.is_terminal(), f"is_terminal={tracker.is_terminal()} (NEW非终态)")
...
check(
    "7.1 交易所撤单",
    cancel_result.get("status") == "CANCELED",
...
check("7.2 状态机完成", tracker.status.value == "CANCELED", f"status={tracker.status.value}")
...
if isinstance(history, dict) and "error" in history:
    time.sleep(1)
    history = binance_call(
        "/fapi/v1/allOrders", signed=True, params={"symbol": "BTCUSDT", "orderId": order_id, "limit": 5}
    )
hist_status = history[0].get("status") if isinstance(history, list) and history else "query_failed"
check(
    "7.5 历史记录确认",
    hist_status == "CANCELED",
...
system_facts = AccountFactSnapshot(
    ...
    positions={
        InstrumentId("BTCUSDT"): Quantity(
            amount=str(next((float(p["positionAmt"]) for p in positions if p["symbol"] == "BTCUSDT"), 0.0))
        )
    },
...
check(
    "8.3 对账结果",
    True,  # open_orders mismatch expected (我们记录了order_id, 交易所已取消)
...
check(
    "9.2 账户余额不变",
    balance_diff < 5.0,  # 允许持仓浮动盈亏导致的小幅变动
    f"before={total_balance:.4f} after={final_balance:.4f} diff={balance_diff:.4f} (正常浮动)",
)

# 确认 canTrade 正常
check("9.3 交易权限正常", account_info.get("canTrade"))
